...
- The three product pillars map to the three buying triggers found in research: confidentiality (NDAs/PIPEDA/PHIPA), grounding in private documents, and auditability (PHIPA now requires electronic audit logs, so traceability is a compliance feature, not a slogan).
...
|---|---|---|---|
| **Mining supply & services** | 300+ firms locally (~14k jobs, ~$4B exports, per Invest Sudbury); mid-size and privately held (fast decisions); document-heavy (safety, ISO, manuals, tenders); NDA-bound to majors, so real privacy constraint; founder has mining-software credibility (LoopX, Vale app); reachable via MineConnect/NORCAT | Not all firms are tech-forward; some data is paper | **Primary wedge** |
...
- Sudbury cluster: 300+ MSS firms, ~14k jobs, ~$4B exports, per Invest Sudbury.
- PHIPA requires electronic audit logs on PHI systems; 2026 compliance coverage.
- GPU price ranges, from multiple public price sheets.
- Founder delivery history (Flosonics warehouse + AI assistant, Vale offline app, OreAcle offline POC) is real, and OreAcle is publicly linked.

**Defensible but softer (framed with hedges on the site):**
- Implementation cost ranges are grounded in published benchmarks, but a solo founder's actual costs will vary; framed as "planning ranges, not quotes."
- "~40% reporting overhead reduction" at Flosonics is real experience but internal/unauditable; kept, attributed to past employment only.
- "2–4 weeks to POC" is realistic for the scoped green-column offer, but the first live customer will find friction; the scope box exists to protect this claim.
...
3. **Pick 1 POC** against the fit criteria on the site; resist taking a bad-fit freebie, since the evaluation report is the real product of this phase.
...
- **Model churn:** open-weight leaders change quarterly; mitigation: the model is a swappable component, so churn is an upgrade path, not a rebuild.
